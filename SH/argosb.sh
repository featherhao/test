...
EOF
    ...
    for sel in "${selections[@]}"; do
        case $sel in
            ...
            12)
                ...
                read -rp "输入 Argo 固定隧道域名 agn (CF解析域名): " agn
    ...
    if ! argosb_status_check; then
        ...
        info "🔹 ArgoSB 已安装，正在更新协议 (带 rep)..."
        bash <(curl -Ls "$SCRIPT_URL") rep $VAR_STR
        info "✅ 协议更新完成"
    fi
}
...
update_script() { bash <(curl -Ls "$SCRIPT_URL") rep; info "脚本已更新"; }
...
change_port() { read -rp "请输入协议标识 (例如 xhpt): " proto; read -rp "请输入新的端口号: " port; bash <(curl -Ls "$SCRIPT_URL") rep "$proto=$port"; }
