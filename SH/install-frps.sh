...
get_frps_image() {
    local version=$1
    get_arch
    if [[ "$version" == "0.65.0" ]]; then
        case "$DOCKER_ARCH" in
            amd64) IMAGE="ghcr.io/fatedier/frps:v0.65.0@sha256:5c0755b887dc051dc5a7164ab4849ec151ac3b447f7e87e1b9df5ff03de8a8e4" ;;
            arm64) IMAGE="ghcr.io/fatedier/frps:v0.65.0@sha256:a25093a82412bc3b9118a08a55ef4766ef84b52fd2d9de596d7124ae2d446218" ;;
            arm32) IMAGE="ghcr.io/fatedier/frps:v0.65.0@sha256:2bd9397e0e8cdac37eef0d3f8735b758890702afcd6066e9757bd03aebd030e4" ;;
        esac
    elif [[ "$version" == "0.20.0" ]]; then
        # 固定 0.20.0 版本镜像（如果官方无 Docker，可用 Dockerfile 或自建）
        case "$DOCKER_ARCH" in
        ...
        echo -e "${C_RED}❌ 不支持的版本: $version${C_RESET}"; exit 1
    fi
}
...
uninstall_frps() {
    ...
    get_frps_image "0.65.0"; docker rmi "$IMAGE" >/dev/null 2>&1 || true
