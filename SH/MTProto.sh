...
# 统一错误处理
trap 'status=$?; line=${BASH_LINENO[0]}; echo -e "\e[31m❌ 发生错误 (exit=$status) at line $line\e[0m"; exit $status' ERR
...
# 生成官方推荐的 MTProxy secret
generate_secret() {
    # 16 字节随机 + 16 字节 Telegram tag
    HEX=$(head -c 16 /dev/urandom | xxd -p)
    echo "${HEX}6f6d656761746167"  # 这里 6f6d656761746167 是示例 tag，可以换成 Telegram 官方 tag
}
...
install_mtproxy() {
    ...
    echo -e "${C_GREEN}[INFO] MTProxy 安装完成！${C_RESET}"
